...
@section('content')
    <div class="container-fluid py-6 px-md-4">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
            ...
            <form method="GET" action="{{ url()->current() }}" class="mb-6 flex flex-col gap-4" id="filter-form">
                <div class="flex items-end gap-4 w-full">
                    <div class="flex-1">
                        <label for="filter-name" class="block text-sm font-medium text-gray-700 mb-1">Фильтр по
                            названию</label>
                        <input type="text" id="filter-name" name="filter" value="{{ old('filter', $filter) }}"
                    ...
                    <div>
                        <a href="{{ url()->current() }}"
                            class="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-800 rounded-md transition duration-200 inline-block">
                            Сбросить
                        </a>
                    </div>
                </div>
            ...
            @else
                ...
                <div class="mt-6 flex justify-center" id="pagination">
                    <nav class="inline-flex rounded-md overflow-hidden shadow-sm" role="navigation"
                        ...
                        @foreach ($paginatedItems->appends(['filter' => $filter])->getUrlRange(1, $paginatedItems->lastPage()) as $page => $url)
                            @if ($page == $paginatedItems->currentPage())
                                <span class="px-4 py-2 bg-[#6E76C1] text-white font-medium">{{ $page }}</span>
                            @else
                                <a href="{{ $url }}"
                                    class="px-4 py-2 bg-white text-gray-700 border border-gray-300">{{ $page }}</a>
                            @endif
                        @endforeach
